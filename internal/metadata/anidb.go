...
var defaultAniDBResolver = NewAniDBResolver()
...
func (r *AniDBResolver) Resolve(ctx context.Context, titles []string) (AniDBMatch, error) {
	if err := r.ensureLoaded(ctx); err != nil {
		return AniDBMatch{}, err
	}
	r.mu.RLock()
	defer r.mu.RUnlock()
	for _, title := range titles {
		if match, ok := r.titles[normalizeTitle(title)]; ok {
			return match, nil
		}
	}
...
func (r *AniDBResolver) ensureLoaded(ctx context.Context) error {
	...
	if lastErr != nil && time.Now().Before(retryAt) {
		return lastErr
	}

	r.mu.Lock()
	defer r.mu.Unlock()
	if len(r.titles) > 0 && time.Since(r.loadedAt) < 24*time.Hour {
		return nil
	}
	if r.lastError != nil && time.Now().Before(r.retryAt) {
		return r.lastError
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		err = fmt.Errorf("AniDB titles: HTTP %d", resp.StatusCode)
		r.rememberError(err)
		return err
	}
	...
	for {
		...
		if decodeErr != nil {
			if strings.Contains(strings.ToLower(decodeErr.Error()), "eof") {
				break
			}
			r.rememberError(decodeErr)
			return decodeErr
		}
		...
		for _, title := range anime.Titles {
			...
			if canonical == "" || lang == "en" || lang == "x-jat" {
				canonical = text
			}
		}
		if canonical == "" {
		...
		for _, title := range anime.Titles {
			...
			if _, exists := index[key]; !exists {
				index[key] = AniDBMatch{ID: aid, Title: canonical}
			}
		}
	}
	if len(index) == 0 {
		err = errors.New("AniDB titles: empty index")
		r.rememberError(err)
		return err
	}
	r.titles = index
