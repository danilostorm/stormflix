...
func init() {
	for i := range providerDefinitions {
		...
		case "screenscraper":
			...
			d.Description = "Identificação retro por hashes + títulos, capas, screenshots, regiões e metadados."
		...
		case "retroachievements":
			d.Stage = "configuravel"
			d.Description = "Metadados e conquistas quando o jogo possui ID RetroAchievements verificado por hash/Hasheous."
		...
		case "hasheous":
			...
			d.Fields = []ProviderField{
				{Key: "base_url", Label: "URL da API", Required: true, Placeholder: "https://hasheous.org/api/v1"},
				{Key: "api_key", Label: "Client API Key", Secret: true, Required: true, Placeholder: "••••••••"},
			}
		case "thegamesdb":
			d.Stage = "configuravel"
			d.Description = "Fallback de metadados e artwork do TheGamesDB para títulos não resolvidos pelas fontes principais."
			d.Fields = []ProviderField{{Key: "api_key", Label: "API Key", Secret: true, Required: true, Placeholder: "••••••••"}}
		case "libretro":
			d.Stage = "configuravel"
			d.Description = "Artwork de fallback do repositório público Libretro Thumbnails, alinhado ao ecossistema RetroArch."
			d.Fields = nil
		}
	}
}
