/* StormFlix Admin unified operational queue: scans + metadata + marker detection + games. */
(function(){
  let timer=null;
  ...
  function jobCard(j){
    ...
    const isScan=j.kind==='scan'||j.kind==='game_scan';
    const count=j.total>0?`${Number(j.current||0).toLocaleString('pt-BR')} / ${Number(j.total||0).toLocaleString('pt-BR')}`:(isScan?`${Number(j.current||0).toLocaleString('pt-BR')} arquivo(s)`:Number(j.current||0).toLocaleString('pt-BR'));
    return `<article class="job-card ${active?'active':''}"><div class="job-card-head"><div><h3>${esc(j.label)}</h3><p>${esc(j.library)} · ${esc(j.kind)}</p></div><span class="job-status ${esc(j.status)}">${esc(statusLabel(j.status))}</span></div><div class="job-progress"><span style="width:${active&&pct===0?3:pct}%"></span></div><div class="job-card-meta"><span>${count}${j.total>0?` · ${pct}%`:''}</span><span>OK ${Number(j.success||0).toLocaleString('pt-BR')} · Erros ${Number(j.failed||0).toLocaleString('pt-BR')}</span></div><div class="job-message">${esc(j.message||'')}</div>${isScan&&active?`<div class="actions" style="margin-top:10px"><button class="danger" data-cancel-scan="${j.library_id}">Cancelar scan</button></div>`:''}</article>`;
  }

  async function scanAll(){
    if(!confirm('Colocar todas as bibliotecas ativas na fila de scan? Mídia e jogos usarão seus scanners próprios.'))return;
  ...
  async function decorateLibraryQueue(){
    ...
    try{
      ...
      const scanJobs=jobs.filter(j=>(j.kind==='scan'||j.kind==='game_scan')&&activeStatus(j.status));
