/* StormFlix Admin unified operational queue: scans + metadata + principal-series refresh. */
(function(){
  let timer=null;
  ...
  function renderJobs(root,jobs){
    ...
    root.innerHTML=`<div class="job-queue-head"><div><p class="kicker">TRABALHOS EM SEGUNDO PLANO</p><h2>Fila & atividades</h2><p>Acompanhe scans, metadados e a reorganização de episódios após corrigir uma obra principal.</p></div><div class="job-queue-summary"><span>${running} executando</span><span>${queued} na fila</span><span>${errors} com alerta</span></div></div><div class="panel"><div class="panel-head"><div><h2>Fila operacional</h2><small>Scans de bibliotecas são processados um por vez para não sobrecarregar Drive/rclone e SQLite.</small></div><button class="primary" data-scan-all>Escanear todas</button></div><div class="job-list">${jobs.length?jobs.map(jobCard).join(''):'<div class="job-empty">Nenhum trabalho registrado ainda.</div>'}</div></div>`;
