...
(function(){
  ...
  function install(){
    const next=$('#game-player-overlay');if(!next||next===overlay)return;cleanup();overlay=next;overlay.classList.add('sf-game-g41');
    const kicker=$('.game-player-kicker',overlay);if(kicker)kicker.textContent='STORMFLIX GAME PLAYER G4.1';
    ...
    panelObserver=new MutationObserver(()=>{if(menuOpen())keepChrome();else showChrome()});
  ...
  window.addEventListener('stormflix:game-menu-request',keepChrome);
