...
public final class PlaybackCapabilities {
    ...
    static {
        ...
        VIDEO_MIME_TO_CODEC.put("video/mpeg2", "mpeg2video");
    ...
    public static JSONObject buildRequest(Context context, SessionStore store, String playbackSessionId) throws Exception {
        ...
        JSONObject capabilities = new JSONObject()
            ...
            .put("picture_in_picture", Build.VERSION.SDK_INT >= 26 && !television)
    ...
    private static VideoLimits commonVideoLimits(MediaCodecInfo.VideoCapabilities video) {
        int[][] sizes = {{7680,4320},{4096,2160},{3840,2160},{2560,1440},{1920,1080},{1280,720},{720,480}};
        ...
        for (int[] size : sizes) {
            ...
            for (double rate : rates) {
                try {
                    if (video.areSizeAndRateSupported(size[0], size[1], rate)) {
                        if (!supported) { result.width=size[0]; result.height=size[1]; supported=true; }
            ...
            try {
                if (video.isSizeSupported(size[0],size[1])) { result.width=size[0];result.height=size[1];result.frameRate=24;return result; }
            } catch (Exception ignored) {}
        }
        return result;
    }
    ...
    private static void addHdrTypes(MediaCodecInfo.CodecCapabilities caps,String mime,Set<String> out) {
        ...
        for(MediaCodecInfo.CodecProfileLevel level:caps.profileLevels){int profile=level.profile;
