...
- Web clients now report an explicit local-decode capability contract covering WebAssembly, Worker, WebCodecs, secure context, resolution budget and supported local codecs.
- Added optional HEVC/H.265 compatibility through the MIT-licensed `hevc.js` runtime (`@hevcjs/hlsjs-plugin` 0.1.2 + `@hevcjs/core` 1.4.2). StormFlix keeps the original HEVC video stream copied into its existing fMP4/HLS session while the browser performs the HEVC→browser-compatible decode/transcode work locally.
...
- Browser local-decode budget is conservative and automatic: low-power clients cap at 720p, stronger clients at 1080p and high-core/high-memory desktops advertise up to 3840×2160.
...
- Player diagnostics and Admin expose read-only local HEVC status; local decode and 4K eligibility remain internal automatic PlaybackPlan decisions. HDR local decode remains intentionally disabled until its color pipeline is proven.
...
- Removed the user-facing decode/4K switches and made capable desktops advertise up to 3840×2160 automatically, fixing 3840×1600 HEVC sources incorrectly entering server video transcode solely because a hidden browser preference had not been enabled.
